...
//----------------------------------------------
    ...
    public:
        ...
        static new_handler set_malloc_handler(new_handler f) throw()
        { //類似 C++ 的 set_new_handler().
            // void (*old)() = __malloc_alloc_oom_handler;
            new_handler old = __malloc_alloc_oom_handler;
...
//第二級配置器
//----------------------------------------------
    // 枚举enum相当于给整型的状态量集合起了一个名称, 增强程序可读性, enum只有一个常量时同constant
...
//----------------------------------------------
    ...
    chunk_alloc(size_t size, int& nobjs)
    {
        ...
        if (bytes_left >= total_bytes) {
            ...
            if (0 == start_free) {
                int i;
                // 之所以这么声明是因为free_list是obj* volatile类型, 二者要相加赋值
                obj* volatile *my_free_list;
                obj* p;
